...
def build_cr_summary(repo: str, pr_number: str) -> dict[str, Any]:  # noqa: C901
    ...
    for e, t in session_events:
        # Optimization: Avoid allocating a new list on every iteration for existing keys
        # as dict.setdefault eagerly evaluates the default argument.
        item_id_key = str(e["item_id"])
        if item_id_key not in by_item:
            by_item[item_id_key] = []
        by_item[item_id_key].append((t, e))
