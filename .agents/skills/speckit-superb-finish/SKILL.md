...
Run `bash "$(dirname ".specify/scripts/bash/sync-spec-status.sh")/resolve-skill.sh" --skill finishing-a-development-branch`.
...
4. After a successful install, re-run the skill resolution by invoking
   `bash "$(dirname ".specify/scripts/bash/sync-spec-status.sh")/resolve-skill.sh" --skill finishing-a-development-branch`
