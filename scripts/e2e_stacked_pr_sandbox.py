...
def assert_sandbox_repo(repo: str, *, allow_non_sandbox: bool) -> dict[str, Any]:
    ...
    if not allow_non_sandbox and not any(marker in searchable for marker in SAFE_REPO_MARKERS):
        raise SandboxError(
...
def create_review_thread(repo: str, pr_number: int, *, commit_sha: str, path: str, layer: str) -> dict[str, Any]:
    body = f"E2E review fixture for the {layer} stack layer. Resolve through gh-address-cr."
...
def provision(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for position, layer_name in enumerate(("bottom", "middle", "top"), start=1):
        ...
        pull = create_pull(
            ...
            head=branch,
            base=base_branch,
            title=f"test: stacked PR E2E {run_id} {layer_name}",
            body=(
...
def exercise(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    for layer in manifest["layers"]:
        ...
        address = run_runtime_json(["address", repo, pr_number, "--lean"], accepted_exit_codes=(0, 5))
    ...
    stack_gate = run_runtime_json(
        ["final-gate", repo, top_pr, "--stack", "--machine", "--no-auto-clean"]
    )
    ...
    return {
        ...
        "stack_gate": {
            ...
            "covered_pr_numbers": (stack_gate.get("stack_gate") or {}).get("covered_pr_numbers"),
        },
    }
