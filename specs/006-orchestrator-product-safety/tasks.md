...
- [x] T001 Initialize task tracking in `specs/006-orchestrator-product-safety/tasks.md`
...
- [x] T004 [US1] Inject `reason_code` and `next_action` mapping in `handle_submit`, `handle_step`, and `handle_start` exit paths in `src/gh_address_cr/orchestrator/harness.py`
- [x] T005 [P] [US1] Create unit tests verifying structured error outputs and next actions in `tests/test_orchestrator_harness.py`
...
- [x] T006 [US3] Parse CLI args and ENV vars for `max_concurrency` and `circuit_breaker_threshold` and persist to `config` in `src/gh_address_cr/orchestrator/harness.py`
- [x] T007 [US3] Enforce `max_concurrency` limit during task dispatch in `handle_step` in `src/gh_address_cr/orchestrator/harness.py`
- [x] T008 [US3] Implement Role-Based Visibility filtering in `handle_step` in `src/gh_address_cr/orchestrator/harness.py`
- [x] T009 [US3] Implement circuit breaker logic mapping N retries to `HUMAN_INTERVENTION_REQUIRED` state in `handle_submit` in `src/gh_address_cr/orchestrator/harness.py`
- [x] T010 [US3] Implement manual repair recovery path clearing `waiting_for_human` state upon valid `submit` in `src/gh_address_cr/orchestrator/harness.py`
- [x] T011 [US3] Implement Orchestration Completion Lock logic (`completed: true`) in `handle_stop` in `src/gh_address_cr/orchestrator/harness.py`
- [x] T012 [US3] Implement lock evaluation and auto-clear logic against core truth in `handle_start` and `handle_step` in `src/gh_address_cr/orchestrator/harness.py`
- [x] T013 [P] [US3] Create comprehensive unit tests for coordination guardrails, human intervention, and locking in `tests/test_orchestrator_harness.py`
...
- [x] T015 Run full test suite `python3 -m unittest discover -s tests`
- [x] T016 Final manual smoke test of integrated `agent orchestrate` guardrails
- [x] T017 Verify performance goal: assert status/lock resolution overhead remains <100ms in `tests/test_orchestrator_harness.py`
