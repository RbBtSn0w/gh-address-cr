...
**Created**: 2026-05-30
**Status**: Verified
**Input**: User description: "开发CLI，同时维护skill，毕竟他们的代码分离，一不小心丢失就出错，非常低下。从系统化思考，第一性原理给出复盘和修复方案。短期修复（阶段一）：建立兼容脚本的单一可信源、实现同步逻辑与集成校验以防漂移。中期重构（阶段二）：脚本“极简代理化”（彻底下沉逻辑）。"
...
### User Story 2 - Minimalist Delegation of Legacy Scripts (Phase 2 - In Progress)
...
- **SC-002**: All compatibility scripts under `skill/scripts/` are minimal proxies with zero business logic.
