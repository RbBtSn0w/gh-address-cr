...
2. **Given** a pending task, **When** `orchestrate step` is run, **Then** it calls `workflow.issue_action_request` and generates a `WorkerPacket` containing the runtime-vended request details.
...
**Independent Test**: Can be tested by providing a malformed JSON response file to `submit`; the system must retry parsing up to 3 times (if triggered by a loop) and eventually raise `HumanHandoffRequired`.

**Acceptance Scenarios**:

1. **Given** a response file that is temporarily unreadable, **When** the system attempts to parse it, **Then** it retries up to 3 times.
2. **Given** a response file that is consistently invalid JSON, **When** the system attempts to parse it 3 times, **Then** it fails with a clear instruction for human intervention.
...
- **Inconsistent Queue**: If the core session shows 0 items but the orchestrator queue has 1? (Orchestrator MUST re-sync from runtime truth during every `step`).
- **Missing Workflow Context**: How does `step` handle roles not supported by the current runtime? (Must fail loud or return `READY` if filtering).
...
- **SC-004**: Resuming an orchestration session correctly re-syncs the queue status from the current `session.json` within 1 second.
