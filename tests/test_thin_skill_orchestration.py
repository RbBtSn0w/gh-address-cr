...
class ThinSkillOrchestrationTests(unittest.TestCase):
    ...
    def test_stage_5_scope_guards(self):
        self.assertFalse(
            (SRC_CORE_DIR / "autonomous_runner.py").exists(), "Autonomous runner must not be built in this stage"
        )
        self.assertFalse(
            (SRC_CORE_DIR / "review_generator.py").exists(), "Review generator must not be built in this stage"
        )
