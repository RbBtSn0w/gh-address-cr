...
- Production Homebrew publishing: uses the shared release-bot GitHub App to mint a short-lived token scoped to `RbBtSn0w/homebrew-tap`; install the app on `RbBtSn0w/homebrew-tap` with `Contents: Read and write`, expose `RELEASE_BOT_APP_ID` as an Actions variable, and expose `RELEASE_BOT_PRIVATE_KEY` as an Actions secret to this repository. The workflow renders the formula with the runtime dependency closure, then validates it with `brew audit --formula --strict`, source install, and `brew test` before pushing.
